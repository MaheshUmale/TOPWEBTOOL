...
|------|------|----------------|-------|
| 2026-08-13 | Mascot SVG animation | `site.min.css`, `global.js` | Added `twt-float` + `twt-sparkle` keyframes; applied `#twt-brand-logo` and `.twt-logo-spark` classes to inline SVGs in header/footer |
| 2026-08-13 | Staggered card entrance | `site.min.css`, `global.js` | Added `twt-cardIn` keyframe; `staggerToolCards()` sets `--i` CSS variable per card on homepage only |
...
| 2026-08-13 | Loading skeletons | `site.min.css` | Added `.twt-skeleton` shimmer class for tool pages to adopt |
| 2026-08-13 | prefers-reduced-motion | `site.min.css` | Global media query kills all animation/transition durations |
| 2026-08-13 | Search exit animation | `site.min.css`, `index.html` | Cards animate `twt-cardExit` before `.hidden` is applied |
| 2026-08-13 | Hover morphing | `site.min.css` | Emoji icons wobble on card hover via `twt-iconWobble` keyframe |
| 2026-08-13 | Theme transition choreography | `site.min.css`, `global.js` | Added `twt-themeWipe` opacity pulse; body class toggled with 150ms delay during theme change |
| 2026-08-13 | First-visit coach mark | `site.min.css`, `global.js` | `showCoachMark()` displays contextual tooltip on first visit; auto-dismisses after 5s; stored in `localStorage` |
| 2026-08-13 | Sound toggle + click feedback | `site.min.css`, `global.js` | `setupSoundToggle()` adds speaker button; uses Web Audio API for synthesized click; default off, persisted in `localStorage` |
...
| 2026-08-13 | Onboarding snippets | `site.min.css`, `global.js` | `showOnboardingSnippet()` rotates 5 contextual tips; auto-dismisses after 6s |
| 2026-08-13 | Result storytelling | `global.js` | `addResultStorytelling()` removed — was dead code, never invoked |
| 2026-08-13 | Premium hero section (Halo Effect) | `site.min.css`, `index.html` | Added `.twt-hero` with radial gradient background and `twt-heroIn` entrance animation |
| 2026-08-13 | Premium card hover (Micro-interactions) | `site.min.css` | `.tool-card:hover` lifts with `translateY(-3px)` and elevated shadow |
| 2026-08-13 | Search/input focus glow (Micro-interactions) | `site.min.css` | Brand-colored glow ring on `#directory-search:focus` and `.form-input:focus` |
| 2026-08-13 | Smooth scroll + selection polish (Cognitive Fluency) | `site.min.css` | `html { scroll-behavior: smooth }` and custom `::selection` brand tint |
...
| 2026-08-14 | Glassy / velvet design system | `site.min.css`, `site.min.css` | Translucent surfaces with `backdrop-filter: blur() saturate()`, warm pearl light mode tokens (`#faf9f7`), animated mesh gradient background, velvet grain overlay, inset highlight shadows, premium glass nav/rail modules/cards/buttons, `--transition-velvet` easing |
| 2026-08-14 | Rail toggle visibility fix | `site.min.css` | Collapsed rail toggle now floats fixed at viewport edge with glass pill styling; removed `opacity: 0` cascade issue |
