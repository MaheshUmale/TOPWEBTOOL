...
- Build step: `npm run build` regenerates `styles.css` from `src.css` via Tailwind CLI
...
### `sitemap.xml`
- **509 URLs**: 1 homepage + 84 tool pages + 420 article pages + 4 legal pages
- **Excluded**: `404.html` (noindex via `_headers`)
- **Generated**: Manually maintained or generated by external script (not in repo)
- **Validated by**: `build-deploy.py` — scans every `<tool-slug>/` directory and every article subdirectory to ensure every `index.html` has a matching `<url><loc>` entry

### `llms.txt`
- **84 bullet entries**, one per tool
- Format: `- [Tool Name](https://topwebtool.com/<tool-slug>/) — Description`
- **Validated by**: `build-deploy.py` — ensures every `UTILITIES_REGISTRY` slug appears

### `llms-full.txt`
- **84 tool sections** with full content (heading, description, sections, guide text)
- **Validated by**: `build-deploy.py` — ensures every `UTILITIES_REGISTRY` slug appears
...
## 7. Build & Validation (`build-deploy.py`)

The build script runs these validations **before** creating `deployment.zip`:

1. **`llms.txt`** — all 84 tool slugs + 4 legal pages present
2. **`llms-full.txt`** — all 84 tool slugs present
...
- [ ] `build-deploy.py` — Auto-discovers tools by scanning directories

### Verification after changes

```powershell
python build-deploy.py          # Validates all cross-references + builds ZIP
npx htmlhint                    # Expect: "Scanned 511 files, no errors found"
...
3. **`build-deploy.py` validates cross-references** but does not generate missing entries. It will **fail** if you add a tool folder without updating `sitemap.xml` / `llms.txt` / `llms-full.txt`.
